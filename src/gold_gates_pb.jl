# Autogenerated using ProtoBuf.jl v1.1.1 on 2025-09-11T09:53:26.222
...
XXSolution(;nsteps = zero(UInt32), angle_sign = false, time = Vector{Float64}(), phase = Vector{Float64}(), phase_slope = Vector{Float64}(), amp = Vector{Float64}(), amp_slope = Vector{Float64}()) = XXSolution(nsteps, angle_sign, time, phase, phase_slope, amp, amp_slope)
PB.default_values(::Type{XXSolution}) = (;nsteps = zero(UInt32), angle_sign = false, time = Vector{Float64}(), phase = Vector{Float64}(), phase_slope = Vector{Float64}(), amp = Vector{Float64}(), amp_slope = Vector{Float64}())
...
function PB.encode(e::PB.AbstractProtoEncoder, x::XXSolution)
    ...
    x.angle_sign != false && PB.encode(e, 2, x.angle_sign)
...
function PB._encoded_size(x::XXSolution)
    ...
    x.angle_sign != false && (encoded_size += PB._encoded_size(x.angle_sign, 2))
