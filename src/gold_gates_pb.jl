# Autogenerated using ProtoBuf.jl v1.2.0 on 2025-11-20T14:48:17.510
# original file: /Users/vivz/Documents/git/GoldGates.jl/pb/gold_gates.proto (proto3 syntax)
...
XXSolution(;nsteps = zero(UInt32), angle_sign = zero(Int32), time = Vector{Float64}(), phase = Vector{Float64}(), phase_slope = Vector{Float64}(), amp = Vector{Float64}(), amp_slope = Vector{Float64}(), metadata = "") = XXSolution(nsteps, angle_sign, time, phase, phase_slope, amp, amp_slope, metadata)
PB.default_values(::Type{XXSolution}) = (;nsteps = zero(UInt32), angle_sign = zero(Int32), time = Vector{Float64}(), phase = Vector{Float64}(), phase_slope = Vector{Float64}(), amp = Vector{Float64}(), amp_slope = Vector{Float64}(), metadata = "")
PB.field_numbers(::Type{XXSolution}) = (;nsteps = 1, angle_sign = 2, time = 3, phase = 4, phase_slope = 5, amp = 6, amp_slope = 7, metadata = 8)
...
function PB.decode(d::PB.AbstractProtoDecoder, ::Type{<:XXSolution})
    ...
    return XXSolution(nsteps, angle_sign, time[], phase[], phase_slope[], amp[], amp_slope[], metadata)
end
...
function PB.encode(e::PB.AbstractProtoEncoder, x::XXSolution)
    ...
    !isempty(x.metadata) && PB.encode(e, 8, x.metadata)
    return position(e.io) - initpos
end
function PB._encoded_size(x::XXSolution)
    ...
    !isempty(x.metadata) && (encoded_size += PB._encoded_size(x.metadata, 8))
    return encoded_size
end
